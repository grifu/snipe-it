    ...
    @else
        ...
        <div class="col-md-7{{  ((isset($required)) && ($required=='true')) ? ' required' : '' }}">     
            @if ($fieldname == 'responsible')
                <select class="js-data-ajax" data-endpoint="responsible" data-placeholder="{{ trans('general.select_user') }}" name="{{ $fieldname }}" style="width: 100%" id="{{ (isset($user_id)) ? $user_id : '1' }}" >
            @else
